...
namespace Joomla\Component\Guided\Administrator\Model;
 ...
 * @since  4.0.0
 */
class GuidedtourModel extends ListModel
{
	/**
	 * Constructor.
	 *
	...
	protected function countItems($items)
	{
		...
		foreach ($items as $item)
		{
			if (isset($status[$item->id]))
			{
				 ...
				 * camel case
				 */
				$item->countStates = (int) $status[$item->id]->count;
			}
			...
			if (isset($transitions[$item->id]))
			{
				 ...
				 * Camel Case
				 */
				$item->countTransitions = (int) $transitions[$item->id]->count;
			}
		}
	}
